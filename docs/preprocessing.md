...
We outline steps to transform raw chess frames into structured data. The identical preprocessing is used during model evaluation with new frames.
...
Splitting the board image into tiles simplifies classification. Processing the whole board would require labeling every possible board state, which is unfeasible. By extracting individual tiles, we dramatically reduce label complexity and can augment training data efficiently using fewer board images.
...
The `build_dataset.py` script uses a YAML configuration file to define data splitting rules, such as proportions for the train, validation, and test sets, or to explicitly list which games go into each set. The split is done at the game level to avoid data leakage.

By splitting data at the game level, all frames from a single game remain together in a single set, helping avoid data leakage and overfitting for reliable evaluation.
